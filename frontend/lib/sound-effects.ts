...
const RECEIVED_BACKGROUND_SOUND_URL = "/received-background.mp3"; // Sound for received payment requests (Background sound)
...
export const playSound = (type: 'notification' | 'message' | 'received' | 'success' | 'request_success' | 'rejected' | 'received_background') => {
    try {
        let url = "";
        switch (type) {
            ...
            case 'received_background': url = RECEIVED_BACKGROUND_SOUND_URL; break;
            case 'request_success': url = REQUEST_SUCCESS_URL; break;
            case 'rejected': url = REJECTED_SOUND_URL; break;
        }
        const audio = new Audio(url);
        audio.volume = (type === 'received' || type === 'received_background') ? 0.5 : 1.0;
